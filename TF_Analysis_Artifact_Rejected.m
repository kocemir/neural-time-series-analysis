...
%     
    ...
    cond=conditions(cn);
     ...
     temppower = mean(abs(eegconv(:,:, ((subject.EEG{1,sb}.eventCodes==cond & ~subject.EEG{1,sb}.reject.rejmanual) | ( subject.EEG{1,sb}.eventCodes==cond+1 & ~subject.EEG{1,sb}.reject.rejmanual) )  )).^2,3); 
     eegpower(:,:,countCond,fi) = 10*log10(temppower./mean(temppower(:,baseidx(1):baseidx(2)),2));
...
subplot(2,2,pos)
contourf(subject.EEG{sb}.times,subject.frex{sb},squeeze(subject.eeg_power{sb}(22,:,cn,:))',60,'linecolor','none')
...
chan2look = [ 9 10 12  13 18]; % average all the channels
...
set(gca,'clim',[-3 3],'xlim',[-500 1000],'yscale','log','ytick',logspace(log10(min_freq),log10(max_freq),6),'yticklabel',round(logspace(log10(min_freq),log10(max_freq),6)*10)/10)
legend(legatns(cn))
